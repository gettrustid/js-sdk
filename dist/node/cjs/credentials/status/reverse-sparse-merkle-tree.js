...
class RHSResolver {
    ...
    async getStatus(credentialStatus, issuerDID, issuerData, genesisState) {
        const issuerId = js_iden3_core_custom_1.DID.idFromDID(issuerDID);
        let latestState;
        try {
...
function isIssuerGenesis(issuer, state) {
    const did = js_iden3_core_custom_1.DID.parse(issuer);
    ...
    const arr = js_iden3_core_custom_1.BytesHelper.hexToBytes(state);
    const stateBigInt = js_iden3_core_custom_1.BytesHelper.bytesToInt(arr);
    const type = (0, js_iden3_core_custom_1.buildDIDType)(method, blockchain, networkId);
    return isGenesisStateId(js_iden3_core_custom_1.DID.idFromDID(did).bigInt(), stateBigInt, type);
}
