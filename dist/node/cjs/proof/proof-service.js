...
class ProofService {
    ...
    async generateProof(proofReq, identifier, opts) {
        ...
        const subjectDID = js_iden3_core_custom_1.DID.parse(preparedCredential.credential.credentialSubject['id']);
        ...
        if (subjectGenesisDID.string() !== genesisDID.string()) {
            throw new Error(verifiable_1.VerifiableConstants.ERRORS.PROOF_SERVICE_PROFILE_GENESIS_DID_MISMATCH);
        }
        const propertiesMetadata = (0, common_1.parseCredentialSubject)(proofReq.query.credentialSubject);
        if (!propertiesMetadata.length) {
            throw new Error(verifiable_1.VerifiableConstants.ERRORS.PROOF_SERVICE_NO_QUERIES_IN_ZKP_REQUEST);
        }
        const mtPosition = preparedCredential.credentialCoreClaim.getMerklizedPosition();
        let mk;
        if (mtPosition !== js_iden3_core_custom_1.MerklizedRootPosition.None) {
            ...
            case circuits_1.CircuitId.AuthV2:
                {
                    const challenge = opts.challenge
                        ? js_iden3_core_custom_1.BytesHelper.intToBytes(opts.challenge).reverse()
                        : new Uint8Array(32);
    ...
    async generateAuthV2Inputs(hash, did, circuitId) {
        ...
        const challenge = js_iden3_core_custom_1.BytesHelper.bytesToInt(hash.reverse());
        ...
        const id = js_iden3_core_custom_1.DID.idFromDID(genesisDID);
    ...
    async verifyState(circuitId, pubSignals, opts = {
        ...
        if (globalStateInfo.replacedByRoot !== 0n) {
            ...
            const timeDiff = Date.now() -
                (0, js_iden3_core_custom_1.getDateFromUnixTimestamp)(Number(globalStateInfo.replacedAtTimestamp)).getTime();
            if (timeDiff >
    ...
    async findCredentialByProofQuery(did, query) {
        ...
        if (!credentials.length) {
            throw new Error(verifiable_1.VerifiableConstants.ERRORS.PROOF_SERVICE_NO_CREDENTIAL_FOR_IDENTITY_OR_PROFILE);
        }
