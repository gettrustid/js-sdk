...
class FetchHandler extends message_handler_1.AbstractMessageHandler {
    ...
    async handleOnchainOfferMessage(offerMessage) {
        ...
        for (const credentialInfo of offerMessage.body.credentials) {
            const issuerDID = js_iden3_core_custom_1.DID.parse(offerMessage.from);
            const userDID = js_iden3_core_custom_1.DID.parse(offerMessage.to);
    ...
    async handleOfferMessage(offerMessage, ctx) {
        ...
        for (const credentialInfo of offerMessage.body.credentials) {
            ...
            const senderDID = js_iden3_core_custom_1.DID.parse(offerMessage.to);
    ...
    async handleFetchRequest(msgRequest) {
        ...
        const issuerDID = js_iden3_core_custom_1.DID.parse(msgRequest.to);
        const userDID = js_iden3_core_custom_1.DID.parse(msgRequest.from);
